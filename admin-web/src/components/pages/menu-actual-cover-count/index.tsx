...
const MenuActualCoverCountPage: React.FC<Props> = ({ operationBranchList }) => {
...
실식수 값: ${row.actualCoverCount}`)
    ...
    try {
      const res = await fetch(`${API_URL}/menu/${menuId}`, {
        ...
        body: JSON.stringify({
          actualCoverCount: Number(row.actualCoverCount),
        }),
      });
  ...
  const columns = [
    ...
    {
      title: '예상식수',
      dataIndex: 'expectedCoverCount',
      width: '15%',
      editable: false,
    },
    {
      title: '실식수',
      dataIndex: 'actualCoverCount',
      width: '15%',
      editable: true,
    },
    {
...
export default MenuActualCoverCountPage;
